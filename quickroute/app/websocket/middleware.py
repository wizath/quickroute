...
class WebSocketMiddleware:
    ...
    async def process_websocket(self, websocket: WebSocket, connection: WebSocketConnection, call_next: Callable):
        """
    ...
    async def process_message(self, connection: WebSocketConnection, message: Any, call_next: Callable):
        """
...
class WebSocketLoggingMiddleware(WebSocketMiddleware):
    ...
    async def process_websocket(self, websocket: WebSocket, connection: WebSocketConnection, call_next: Callable):
        ...
        logger.info(f"WebSocket connection attempt: {connection.connection_id} from {client_host} ({user_agent})")
...
class WebSocketRateLimitMiddleware(WebSocketMiddleware):
    ...
    async def process_websocket(self, websocket: WebSocket, connection: WebSocketConnection, call_next: Callable):
        ...
        try:
            ...
            self.connection_counts[client_host] = max(0, self.connection_counts.get(client_host, 0) - 1)
...
class WebSocketMiddlewareChain:
    ...
    async def process_websocket(self, websocket: WebSocket, connection: WebSocketConnection, handler: Callable):
    ...
    async def process_message(self, connection: WebSocketConnection, message: Any, handler: Callable):
...
def initialize_default_websocket_middleware():
    ...
    add_websocket_middleware(WebSocketAuthMiddleware(require_auth=getattr(settings, 'WEBSOCKET_REQUIRE_AUTH', False)))
