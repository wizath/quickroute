...
class InfluxDBPlugin(BasePlugin):
    ...
    Provides async InfluxDB client for writing and querying time-series data.
    """
    ...
    async def write_point(
        ...
        Args:
            ...
            fields: Field values (actual data)
            tags: Tag values (indexed metadata)
            timestamp: Point timestamp (defaults to now)
            bucket: Bucket name (defaults to INFLUXDB_BUCKET)
    ...
    async def write_points(self, points: List[Dict[str, Any]], bucket: Optional[str] = None) -> bool:
        ...
        Args:
            points: List of point dictionaries with keys: measurement, fields, tags, timestamp
            bucket: Bucket name (defaults to INFLUXDB_BUCKET)
    ...
    async def query_range(
        ...
        Args:
            measurement: Measurement name
            start: Start time (e.g., "-1h", "-24h", "2025-11-01T00:00:00Z")
            stop: End time (default: "now()")
            filters: Tag filters (e.g., {"sensor_id": "001"})
